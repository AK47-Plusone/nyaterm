...
SSH 仍然是 Dragonfly 最完整的一类会话。除了基础登录外，SSH 连接还可以进一步挂接：
...
如果你第一次接触 Dragonfly，通常建议先把 SSH 连接配置好，再去扩展文件、终端增强和网络能力。
...
Dragonfly 支持导入其他客户端的会话配置，目前可导入：
...
Dragonfly 会为 SSH 主机维护已知主机记录，并提供三种主机密钥策略：
...
如果你只是想在 Dragonfly 里开一个本地 shell，请改用 [会话类型](./session-types) 里的 **本地终端**。
