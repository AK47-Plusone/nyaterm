...
export default function NewItemDialog({ data, onClose, onSuccess }: NewItemDialogProps) {
    ...
    const openLabelKey = isFile ? "fileExplorer.openAfterCreateFile" : "fileExplorer.openAfterCreateFolder";
                        ...
                        <div className="flex-1 space-y-3">
                            ...
                            <div className="grid grid-cols-[auto_1fr_1fr_1fr_1fr] gap-x-2 gap-y-3 text-xs items-center">
                                ...
                                <label className="flex items-center gap-1.5 cursor-pointer">
                                    <Checkbox checked={hasBit(1, 4)} onCheckedChange={(v) => updateBit(1, 4, v === true)} /> R
                                </label>
                                <label className="flex items-center gap-1.5 cursor-pointer">
                                    <Checkbox checked={hasBit(1, 2)} onCheckedChange={(v) => updateBit(1, 2, v === true)} /> W
                                </label>
                                <label className="flex items-center gap-1.5 cursor-pointer">
                                    <Checkbox checked={hasBit(1, 1)} onCheckedChange={(v) => updateBit(1, 1, v === true)} /> X
                                </label>
                                <label className="flex items-center gap-1.5 cursor-pointer">
                                    <Checkbox checked={hasBit(0, 4)} onCheckedChange={(v) => updateBit(0, 4, v === true)} /> UID
                                </label>

                                <span className="text-muted-foreground mr-2">{t("fileExplorer.permGroup")}</span>
                                <label className="flex items-center gap-1.5 cursor-pointer">
                                    <Checkbox checked={hasBit(2, 4)} onCheckedChange={(v) => updateBit(2, 4, v === true)} /> R
                                </label>
                                <label className="flex items-center gap-1.5 cursor-pointer">
                                    <Checkbox checked={hasBit(2, 2)} onCheckedChange={(v) => updateBit(2, 2, v === true)} /> W
                                </label>
                                <label className="flex items-center gap-1.5 cursor-pointer">
                                    <Checkbox checked={hasBit(2, 1)} onCheckedChange={(v) => updateBit(2, 1, v === true)} /> X
                                </label>
                                <label className="flex items-center gap-1.5 cursor-pointer">
                                    <Checkbox checked={hasBit(0, 2)} onCheckedChange={(v) => updateBit(0, 2, v === true)} /> GID
                                </label>

                                <span className="text-muted-foreground mr-2">{t("fileExplorer.permOther")}</span>
                                <label className="flex items-center gap-1.5 cursor-pointer">
                                    <Checkbox checked={hasBit(3, 4)} onCheckedChange={(v) => updateBit(3, 4, v === true)} /> R
                                </label>
                                <label className="flex items-center gap-1.5 cursor-pointer">
                                    <Checkbox checked={hasBit(3, 2)} onCheckedChange={(v) => updateBit(3, 2, v === true)} /> W
                                </label>
                                <label className="flex items-center gap-1.5 cursor-pointer">
                                    <Checkbox checked={hasBit(3, 1)} onCheckedChange={(v) => updateBit(3, 1, v === true)} /> X
                                </label>
                                <label className="flex items-center gap-1.5 cursor-pointer">
                                    <Checkbox checked={hasBit(0, 1)} onCheckedChange={(v) => updateBit(0, 1, v === true)} /> {t("fileExplorer.permSticky")}
                                </label>
                            </div>
