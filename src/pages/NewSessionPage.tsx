  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const connection: SavedConnection = {
        id: initialData?.id || "",
        name: name || (currentTab === "local" ? t("dialog.localTerminal") : currentTab === "serial" ? serialPortName : `${host}:${port}`),
    ...
    <div className="h-screen flex flex-col overflow-hidden" data-tauri-drag-region>
      ...
      <Tabs value={currentTab} onValueChange={setCurrentTab} className="flex-1 flex flex-col overflow-hidden">
        <div className="px-5 pt-3 shrink-0">
          <TabsList variant="line" className="w-full justify-start border-b border-border/40 gap-6 rounded-none h-auto p-0">
            <TabsTrigger value="ssh" className="px-1 py-2 text-xs rounded-none bg-transparent">SSH</TabsTrigger>
            <TabsTrigger value="local" className="px-1 py-2 text-xs rounded-none bg-transparent">{t("dialog.localTerminal")}</TabsTrigger>
            <TabsTrigger value="telnet" className="px-1 py-2 text-xs rounded-none bg-transparent">Telnet</TabsTrigger>
            <TabsTrigger value="serial" className="px-1 py-2 text-xs rounded-none bg-transparent">Serial</TabsTrigger>
          </TabsList>
        </div>
        ...
        <div className="flex-1 overflow-y-auto w-full p-5 space-y-4">
          ...
          <TabsContent value="ssh" className="space-y-4 m-0 border-0 outline-none w-full">
            <SshForm host={host} setHost={setHost} port={port} setPort={setPort} username={username} setUsername={setUsername} authType={authType} setAuthType={setAuthType as any} password={password} setPassword={setPassword} keyId={keyId} setKeyId={setKeyId} />
          </TabsContent>

          <TabsContent value="local" className="space-y-4 m-0 border-0 outline-none w-full">
            <LocalTerminal shellPath={shellPath} setShellPath={setShellPath} workingDir={workingDir} setWorkingDir={setWorkingDir} />
          </TabsContent>
          ...
          <TabsContent value="serial" className="space-y-4 m-0 border-0 outline-none w-full">
            <SerialForm serialPortName={serialPortName} setSerialPortName={setSerialPortName} baudRate={baudRate} setBaudRate={setBaudRate} dataBits={dataBits} setDataBits={setDataBits} parity={parity} setParity={setParity} stopBits={stopBits} setStopBits={setStopBits} />
          </TabsContent>
      ...
      <div className="px-5 py-3 border-t flex justify-end gap-2 shrink-0">
        ...
        <Button size="sm" className="text-xs" onClick={handleSave} disabled={connecting || ((currentTab === "ssh" || currentTab === "telnet") && !host)}>
